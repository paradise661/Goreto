            ...
            @if (!$byte->isEmpty())
                <table class="table">
                    ...
                    <tbody class="table-border-bottom-0">
                        @foreach ($byte as $key => $blg)
                            <tr>
                                <td><strong>{{ $key + $byte->firstItem() }}</strong></td>
                                ...
                                <td>
                                    <a class="btn btn-sm btn-success" href="{{ route('bytesingle', $blg['slug']) }}"
                                        style="float: left;margin-right: 5px;" target="_blank"><i
                                            class="fa-solid fa-eye"></i></a>
                                    <a class="btn btn-sm btn-primary" href="{{ route('bytes.edit', $blg->id) }}"
                                        style="float: left;margin-right: 5px;"><i class="fa-solid fa-pen-to-square"></i></a>

                                    <form class="delete-form" action="{{ route('bytes.destroy', $blg->id) }}"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
