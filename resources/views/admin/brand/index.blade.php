                ...
                <table class="table">
                    ...
                    <tbody class="table-border-bottom-0">
                        @foreach ($brands as $key => $category)
                            <tr>
                                <td><strong>{{ $key + $brands->firstItem() }}</strong></td>
                                ...
                                <td>
                                    <div class="d-flex justify-content-end">
                                        ...
                                        <a class="btn btn-sm btn-primary" href="{{ route('brand.edit', $category->id) }}"
                                            style="margin-right: 5px;"><i class="fa-solid fa-pen-to-square"></i></a>
                                        <form class="delete-form" action="{{ route('brand.destroy', $category->id) }}"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
                            ...
                            @if (count($category->children))
                                @include('admin.brand.table.children', [
