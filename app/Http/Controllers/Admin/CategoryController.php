...
class CategoryController extends Controller
{
    ...
    $subCategories = Category::where('parent_id', $id)->with('children')->get();

    return view('admin.category.subcategories', compact('parent', 'subCategories'));
    }
    public function subcategories($parentId)
    {
    $parent = Category::findOrFail($parentId);
    $subCategories = Category::where('parent_id', $parent->id)->orderBy('name')->get();
    ...
    public function products($id)
    {
        ...
        // Assuming a Category hasMany Products
        $products = $category->products()->paginate(20); // adjust pagination as needed
